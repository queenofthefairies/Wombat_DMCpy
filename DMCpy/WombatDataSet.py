...
class WombatDataSet(object):
    def __init__(self, dataFiles=None,unitCell=None,forcePowder=False,**kwargs):
        """WombatDataSet object to hold a series of WombatDataFile objects
        Kwargs:
        ...
        if dataFiles is None:
            ...
            if isinstance(dataFiles,(str,WombatDataFile.WombatDataFile)): # If either string or DataFile instance wrap in a list
                dataFiles = [dataFiles]
            try:
                self.dataFiles = [WombatDataFile.loadWombatDataFile(dF,unitCell=unitCell,forcePowder=forcePowder,**kwargs) if isinstance(dF,(str)) else dF for dF in dataFiles]
            except TypeError:
    ...
    def append(self,item):
        try:
            if isinstance(item,(str,WombatDataFile.WombatDataFile)): # A file path or DataFile has been provided
                item = [item]
            for f in item:
                if isinstance(f,str):
                    f = WombatDataFile.loadWombatDataFile(f)
    ...
    def generateMask(self,maskingFunction = WombatDataFile.maskFunction, replace=True, **pars):
