...
def loadWombatDataFile(fileLocation=None,fileType='Unknown',unitCell=None,forcePowder=False,sampleRotationAxis=None,**kwargs):
    ...
    T = 'Unknown' # type of WombatDataFile

    if A3 is None: # there is no A3 values at all
        T = 'powder'
        
    elif (len(A3) == 1) or forcePowder:# and se_r.any() is None) or forcePowder:
        T = 'powder'
    else:
        T = 'singlecrystal'
    ...
    if not 'radius' in kwargs:
        kwargs['radius'] = 0.728 # Sample-detector distance for Wombat
...
class WombatDataFile(object):
    ...
    def __init__(self, file=None,unitCell=None,forcePowder=False, wavelength=2.41, sampleRotationAxis=None):
        print()
        print('~~~~~~~ Loading Wombat single crystal data ~~~~~~~')
        ...
        if not file is None: 
            if isinstance(file,WombatDataFile): # Copy everything from provided file
    ...
    def loadFile(self,filePath,unitCell=None,wavelength=2.41, forcePowder=False):
        ...
        with hdf.File(filePath,mode='r') as f:
            ...
            # this is for Wombat in standard resolution mode
            self.countShape = (A3_steps, 129, 968) #f.get(HDFCounts).shape
            self.hasBackground = 0 
    ...
    def twoThetaPosition(self,twoTheta):
        ...
        #self.twoTheta = np.repeat((np.linspace(0,-132,1152) + self._detector_position + self._twoThetaOffset)[np.newaxis],self.countShape[1],axis=0) # DMC detector
        self.twoTheta = np.repeat((np.linspace(0,120,968) + self._detector_position + self._twoThetaOffset)[np.newaxis],self.countShape[1],axis=0) # Wombat detector
    ...
    def generateMask(self,maskingFunction = maskFunction, replace=True, **pars):
        ...
        if len(self.countShape) <1:#not hasattr(self,'counts'):
            raise RuntimeError('WombatDataFile does not contain any counts. Look for self.counts but found nothing.')
    ...
    def save(self,filePath,compression=6):
        ...
        with hdf.File(filePath,'w') as f:
            ...
            f.attrs['instrument'] = np.string_('Wombat')
            f.attrs['owner'] = np.string_('Siobhan Tobin <siobhant@ansto.gov.au>')
            ...
            Wombat = entry.create_group('Wombat')
            Wombat.attrs['NX_class'] = np.string_('NXinstrument')
            
            ANSTO = DMC.create_group('ANSTO')
            ANSTO.attrs['NX_class'] = np.string_('NXsource')
            ANSTO.attrs['name'] = np.string_('ANSTO')
            ANSTO.attrs['type'] = np.string_('Continuous flux research reactor source')
            
            detector = ANSTO.create_group('detector')
            ...
            data.attrs['target'] = np.string_('/entry1/data')

            
            entry['entry1/monitor/bm1_counts'].attrs['units'] = np.string_('counts')
...
class SingleCrystalWombatDataFile(WombatDataFile):
...
class PowderWombatDataFile(WombatDataFile):
    def __init__(self,fileType,*args,**kwargs):
        super(PowderWombatDataFile,self).__init__(fileType,*args,**kwargs)
        self.fileType = 'Powder'

        if 'forcePowder' in kwargs:
            if kwargs['forcePowder']:
                self._counts = np.sum(self.counts,axis=(0,1))
                self.countShape = (1,128,968) # Wombat standard resolution
                self.monitor = np.array([np.sum(self.monitor)])
        self.counts.shape = (1,128,968) # Wombat standard resolution
